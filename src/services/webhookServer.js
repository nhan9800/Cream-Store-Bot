...
export async function startWebhookServer(client = null) {
  ...
  // Backward compatibility for links generated by transcript v1. New archives
  // are never exposed as static files. Legacy HTML may be transparently served
  // from its gzip-compressed form after the startup migration.
  app.get('/transcripts/:filename', (req, res, next) => {
    const filename = String(req.params.filename || '').toLowerCase();
    if (!/^transcript_[a-z0-9_-]{3,160}\.html$/.test(filename)) return next();
    const directory = path.join(process.cwd(), 'data', 'transcripts');
    const plainPath = path.join(directory, filename);
    const gzipPath = `${plainPath}.gz`;
    let selected = null;
    let gzip = false;
    if (fs.existsSync(plainPath)) selected = plainPath;
    else if (fs.existsSync(gzipPath)) { selected = gzipPath; gzip = true; }
    if (!selected) return res.status(404).send('Transcript not found');
    res.set({
    ...
    return fs.createReadStream(selected).on('error', next).pipe(res);
  });
