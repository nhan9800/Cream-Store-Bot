  ...
  try {
    ...
    if (interaction.customId === 'otp:select_service') {
      ...
      try {
        ...
        container.addTextDisplayComponents(
          new TextDisplayBuilder().setContent(`## ${E('status_check')} THUÊ SỐ THÀNH CÔNG!\n\n**Dịch vụ:** ${service.name}\n**Số điện thoại:** \`${otpData.phone_number}\`\n**Giá:** ${service.price.toLocaleString('vi-VN')}đ\n\n> ${E('customer_patron')} Bạn đã được ghi nhận là khách hàng Cenar.\n> ${E('status_info')} Sau khi hệ thống gửi mã xác nhận, hãy nhấn nút **Lấy Mã OTP** bên dưới.`)
        );
